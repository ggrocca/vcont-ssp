...
if [ $DO_DEM_FILTER = "yes" ]; then
    ( set -x; $EXE $DEM_OPTS $FILTER_OPTS -i $SOURCE_DIR/$SOURCE_DEM -o  "$RES"/"$DEM"_"$FILTER" -t "$RES"/"$DEM"_"$FILTER" >> "$RES"/"$DEM"_"$FILTER".txt; )
fi

if [ $DO_RANGE_FILTER = "yes" ]; then
    ( set -x; $EXE $RANGE_OPTS $FILTER_OPTS -i $SOURCE_DIR/$SOURCE_RANGE -o  "$RES"/"$RANGE"_"$FILTER" -t "$RES"/"$RANGE"_"$FILTER" >> "$RES"/"$RANGE"_"$FILTER".txt; )
fi
