...
$EXE -r "$RES"/"$RANGE"_"$NORMAL"."$SSP" -o  "$RES"/"$RANGE"_"$READ" -t "$RES"/"$RANGE"_"$READ" > "$RES"/"$RANGE"_"$READ".txt
fi

if [ -n $DO_DEM_READ ]; then
echo $EXE -r "$RES"/"$DEM"_"$NORMAL"."$SSP" -o  "$RES"/"$DEM"_"$READ" -t "$RES"/"$DEM"_"$READ"
$EXE -r "$RES"/"$DEM"_"$NORMAL"."$SSP" -o  "$RES"/"$DEM"_"$READ" -t "$RES"/"$DEM"_"$READ" > "$RES"/"$DEM"_"$READ".txt
fi
