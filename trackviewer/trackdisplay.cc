...
TrackDisplay::TrackDisplay()
{
  ...
  draw_max=draw_min=draw_sel=false;
...
void TrackDisplay::calc_stats()
{
  // for (int i=1; i<track->lines.size(); i++)
  //   {
  //     double s=track->lines[i].scale;
  //     double I=track->lines[i].strength;
  //     if (s<minSCalc) minSCalc=s;
  //     else if (s>maxSCalc) maxSCalc=s;

  //     if (I<minICalc && !isinf(-I)) minICalc=I;
  //     else if (I>maxICalc && !isinf(I)) maxICalc=I;
      
  //     if (isinf(maxICalc)) maxICalc=I;
  //    }
}
...
void TrackDisplay::draw (int dem_idx)
{
  ...
  for (unsigned i = 0; i < track->lines.size(); i++)
    {
      switch(track->original_type(i))
	{
	case MAX:
	  if (!draw_max) break;
	  if (track->is_original (i) && track->lines[i].strength < max_importance[0] && track->lines[i].strength > min_importance[0]  && track->lines[i].scale < max_scale[0] && track->lines[i].scale > min_scale[0] )
	    __draw_critical_elixir (track->lines[i].entries[0].c,
				    track->original_type (i),
	...
	case MIN:
	  if (!draw_min) break;
	  if (track->is_original (i) && track->lines[i].strength < max_importance[1] && track->lines[i].strength > min_importance[1]  && track->lines[i].scale < max_scale[1] && track->lines[i].scale > min_scale[1] )
	    __draw_critical_elixir (track->lines[i].entries[0].c,
				    track->original_type (i),
				    track->lines[i].strength,
				    importance_scale, importance_mult);
	...
	case SA2:
	  if (!draw_sel) break;
	  if (track->is_original (i) && track->lines[i].strength < max_importance[2] && track->lines[i].strength > min_importance[2]  && track->lines[i].scale < max_scale[2] && track->lines[i].scale > min_scale[2] )
	    __draw_critical_elixir (track->lines[i].entries[0].c,
				    track->original_type (i),
				    track->lines[i].strength,
				    importance_scale, importance_mult);
