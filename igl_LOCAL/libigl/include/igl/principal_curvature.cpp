...
public:
  ...
  public:
    ...
    IGL_INLINE static Quadric fit(std::vector<Eigen::Vector3d> &VV, bool zeroDetCheck, bool svd, int fast_computation)
    {
      using namespace std;
      assert(VV.size() >= 5);
      if (VV.size() < 5)
	...
	case 2:
	  ...
	  break;
	default:
	  cerr << "Wrong parameter for fast computation, assuming 0" << endl;
	  sol=A.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(b);
	}
      ...
      // 	  goodnessOfFit += pow(n-nPrime,2);
      // 	} 
      // goodnessOfFit=sqrt(goodnessOfFit);
      // std::cout << goodnessOfFit << std::endl;
...
public:
  ...
  IGL_INLINE void finalEigenStuff (int, std::vector<Eigen::Vector3d> );
  IGL_INLINE void fitQuadric (Eigen::Vector3d, std::vector<Eigen::Vector3d> ref, const  std::vector<int>&, int );
  ...
  IGL_INLINE void computeCurvature(bool, int);
...
//  vertices = vertices.array() * (1.0/igl::avg_edge_length(V,F));
  ...
  t = new TopologyIndex(V,F,4, 2,2, false, true,getAverageEdge() , false, 2, 3);
}

IGL_INLINE void CurvatureCalculator::fitQuadric (Eigen::Vector3d v, std::vector<Eigen::Vector3d> ref, const std::vector<int>& vv, int fast_computation)
{
  ...
  q = Quadric::fit (points, zeroDetCheck, svd, fast_computation);
}

IGL_INLINE void CurvatureCalculator::finalEigenStuff (int i, std::vector<Eigen::Vector3d> ref)
{
...
IGL_INLINE void CurvatureCalculator::computeCurvature(bool topo, int fast_computation)
{
  using namespace std;
  ...
  scaledRadius=/*absoluteValue?*/getAverageEdge()*sphereRadius;//:sphereRadius;
  ...
  clock_t search_b, search_end;
  clock_t ref_b, ref_end;
  clock_t fit_b,fit_e;
  clock_t final_b, final_e;
  ...
  for (size_t i=0; i<vertices_count; ++i)
  {
    ...
    case SPHERE_SEARCH:
      if(topo)
	{
	  level=t->query((int)i,scaledRadius,vv);
	  t->getLevel(level)->markQuery(false);
	}
      else
    ...
    search_end = clock();
    searchtime+=search_end-search_b;    
    std::vector<Eigen::Vector3d> ref(3);
    if (vv->size()<6)
    {
      std::cerr << "Could not compute curvature of radius " << scaledRadius << " at vertex " << i << "Vertices: " << vv->size() << endl;
      return;
    }
    switch (nt)
    {
    ...
    if (vv->size()<6)
    {
      std::cerr << "Could not compute curvature of radius " << scaledRadius  << " at vertex " << i  << endl;
      return;
    }
    if (montecarlo)
    {
    ...
    ref_end = clock();
    ref_time+=ref_end-ref_b;    
    
    fit_b = clock();
    fitQuadric (me, ref, *vv, fast_computation);
    fit_e = clock();
    fit_time+=fit_e-fit_b;
    
    final_b = clock();
    finalEigenStuff(i,ref);
    final_e = clock();
    final_time+=final_e-final_b;

  }
  
  end = clock();
  time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
  searchtime=(double)searchtime / CLOCKS_PER_SEC;
  ref_time=(double)ref_time / CLOCKS_PER_SEC;
  fit_time=(double)fit_time / CLOCKS_PER_SEC;
  final_time=(double)final_time / CLOCKS_PER_SEC;
  std::cout << "TOTAL TIME: " << time_spent << std::endl;
  std::cout << "SEARCH TIME: " << searchtime << std::endl;
