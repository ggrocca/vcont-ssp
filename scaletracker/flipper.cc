...
void Flipper::track (ScaleSpace& ss, Track* track, TrackOrdering* order)
{
    // (void) ss;
...
/// add a single flip
void pair_insert (std::vector<TrackLine>& lines,
		  std::vector<FlippingEvent>& events,
		  Map* map, Flip f,
		    ...
		    int base, TrackString& ts)
{
	...
	pair_insert (lines, events, map, flips[i], before, after, base);
...
// 1M -> RR
// M1 -> RR
void death_1 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	      TrackEntry te0, TrackEntry te1)
{
    ...
    event->type = DEATH_1;
    event->track_ptrs.push_back (TrackEntryPointer (i0, lines[i0].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (i1, lines[i1].entries.size()-1));
}
...
// m2 -> R1
// 2M -> 1R
void death_2 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	      ...
	      CriticalType ct1, TrackEntry ce1)
{
    ...
    event->type = DEATH_2;
    event->track_ptrs.push_back (TrackEntryPointer (is, lines[is].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (is_mate, lines[is_mate].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (im, lines[im].entries.size()-1));
}
...
// RR -> m1
// RR -> 1m
void birth_1 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	      ...
	      CriticalType ct1, TrackEntry te1)
{
    ...
    event->type = BIRTH_1;
    event->track_ptrs.push_back (TrackEntryPointer (lines.size() - 2, 0));
    event->track_ptrs.push_back (TrackEntryPointer (lines.size() - 1, 0));
}
...
// 1R -> 2m
// R1 -> m2
void birth_2 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	      CriticalType ct0, TrackEntry te0,
	      CriticalType ct1, TrackEntry te1)
{
    ...
    event->type = BIRTH_2;
    event->track_ptrs.push_back (TrackEntryPointer (i_ps, lines[i_ps].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (i_te0, lines[i_te0].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (i_te1, lines[i_te1].entries.size()-1));
}
...
// MR -> RM
// RM -> MR
void move_1 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	     TrackEntry from_e, TrackEntry to_e)
{
    ...
    event->type = MOVE_1;
    event->track_ptrs.push_back (TrackEntryPointer (i, lines[i].entries.size()-1));
}

// 12 -> 21
// 21 -> 12
void move_3 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	     TrackEntry from_ce, TrackEntry to_ce)
{
    ...
    event->type = MOVE_3;
    event->track_ptrs.push_back (TrackEntryPointer (old_mate_i, lines[old_mate_i].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (new_mate_i, lines[new_mate_i].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (moving_i, lines[moving_i].entries.size()-1));
}

// R2 -> 11
// 2R -> 11
void move_21 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	     TrackEntry from_ce, TrackEntry to_ce)
{
...
// 11 -> R2
// 11 -> 2R
void move_12 (std::vector<TrackLine>& lines, FlippingEvent* event, Map* map,
	     TrackEntry from_ce, TrackEntry to_ce)
{
    TrackEntry moving_ce = to_ce;
    ...
    event->type = MOVE_12;
    event->track_ptrs.push_back (TrackEntryPointer (to_i, lines[to_i].entries.size()-1));
    event->track_ptrs.push_back (TrackEntryPointer (from_i, lines[from_i].entries.size()-1));
}
		  ...
		  CriticalPair before, CriticalPair after, int base)
{
    ...
    TrackEntry el (f.e.l, current_time);
    TrackEntry er (f.e.r, current_time);
	...
	if (bl != SA3 && br == REG)
	{
	    ...
	    move_1 (lines, &event, map, el, er);
	    goto exit_success;
	}
	if (bl == REG && br != SA3)
	{
	    // 1-move from right to left
	    move_1 (lines, &event, map, er, el);
	    goto exit_success;
	}
	if (bl == SA3 && br == SA2)
	{
	    // 3-move from left to right
	    move_3 (lines, &event, map, el, er);
	    goto exit_success;
	}
	if (bl == SA2 && br == SA3)
	{
	    // 3-move from right to left
	    move_3 (lines, &event, map, er, el);
	    goto exit_success;
	}
	...
	{
	    // 1-birth
	    birth_1 (lines, &event, map, al, el, ar, er);
	    goto exit_success;
	}
	if (al == REG && ar == REG)
	{
	    // 1-death
	    death_1 (lines, &event, map, el, er);
	    goto exit_success;
	}
	if ((bl == REG && br == SA2) || (bl == SA2 && br == REG))
	{
	    // 2-birth
	    birth_2 (lines, &event, map, al, el, ar, er);
	    goto exit_success;
	}
	if ((al == REG && ar == SA2) || (al == SA2 && ar == REG))
	{
	    // 2-death
	    death_2 (lines, &event, map, bl, el, br, er);
	    goto exit_success;
	}
	if (bl == SA3 && br == REG)
	{
	    // 21-move from left to right
	    move_21 (lines, &event, map, el, er);
	    goto exit_success;
	}
	if (bl == REG && br == SA3)
	{
	    // 21-move from right to left
	    move_21 (lines, &event, map, er, el);
	    goto exit_success;
	}
	if (al == SA3 && ar == REG)
	{
	    // 12-move from right to left
	    move_12 (lines, &event, map, er, el);
	    goto exit_success;
	}
	if (al == REG && ar == SA3)
	{
	    // 12-move from left to right
	    move_12 (lines, &event, map, el, er);
	    goto exit_success;
	}
